...
- The bot STT uses Deepgram Flux multilingual via `DEEPGRAM_API_KEY` and defaults to `DEEPGRAM_STT_MODEL=flux-general-multi`. The same Flux STT connection handles English and Spanish, with both languages hinted initially and Spanish selected after the caller chooses it.
- The bot TTS uses Pipecat's `ServiceSwitcher`: English is synthesized by `DeepgramFluxTTSService` with token streaming, while Spanish is synthesized by Aura 2 through `DeepgramTTSService`.
- `DEEPGRAM_TTS_VOICE_EN` defaults to `flux-alexis-en`; `DEEPGRAM_TTS_VOICE_ES` defaults to `aura-2-olivia-es`.
- Flux STT owns turn boundaries in both the bot and the scripted websocket client. The pipeline uses explicit external start/stop strategies instead of Pipecat-managed SmartTurn defaults.
...
- After the caller chooses Spanish, the bot narrows Flux STT language hints to Spanish and switches TTS to Aura 2 for the remainder of the call.
